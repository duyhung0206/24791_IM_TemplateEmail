 ...
 */
class Magestore_Inventorypurchasing_Adminhtml_Inpu_PurchaseordersController extends Magestore_Inventoryplus_Controller_Action implements Magestore_Inventoryplus_Controller_Scan {
    /*
     * Menu path
     * 
     * @var string
     */
    protected $_menu_path = 'inventoryplus/purchaseorder';
    ...
    public function movetotrashAction() {
        ...
        if ($status != 5) {
            try {
                $model->setTrash(Magestore_Inventorypurchasing_Model_Purchaseorder::IS_TRASH)->save();
                Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('inventorypurchasing')->__('Purchase order has been move to trash')
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError(
                        Mage::helper('inventorypurchasing')->__('There is error when move purchase order to trash')
                );
                Mage::getSingleton('adminhtml/session')->addError(
                        $e->getMessage()
                );
            ...
            Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('inventorypurchasing')->__('Can not move to trash with purchase order awaiting delivery !')
            );
        }
    ...
    public function recycleAction() {
        ...
        Mage::app()->cleanCache();
        try {
            $model->setTrash(Magestore_Inventorypurchasing_Model_Purchaseorder::IS_NOT_TRASH)->save();
            Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('inventorypurchasing')->__('Purchase order has been restored')
            );
        } catch (Exception $e) {
            Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('inventorypurchasing')->__('There is error when restore purchase order')
            );
            Mage::getSingleton('adminhtml/session')->addError(
                    $e->getMessage()
            );
    ...
    public function confirmAction() {
        ...
        $model = Mage::getModel('inventorypurchasing/purchaseorder')->load($id);
        try {
            $model->setStatus(Magestore_Inventorypurchasing_Model_Purchaseorder::AWAITING_DELIVERY_STATUS)
                    ->save();
            Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('inventorypurchasing')->__('Purchase order has been confirmed.')
            );
    ...
    public function markaspaidAction() {
        ...
        try {
            ...
            Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('inventorypurchasing')->__('Purchase order has been paid.')
            );
            $this->_redirect('*/*/edit', array('id' => $purchaseOrderId));
        } catch (Exception $e) {
            Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('inventorypurchasing')->__('There is error while mark as paid purchase order.')
            );
            Mage::getSingleton('adminhtml/session')->addError(
                    $e->getMessage()
            );
     ...
     * Michael 201602
     */
    public function completepurchaseorderAction() {
        $purchaseOrderId = $this->getRequest()->getParam('id');
        $purchaseOrder = Mage::getModel('inventorypurchasing/purchaseorder')->load($purchaseOrderId);
        try {
            $purchaseOrder->setCompleteBefore(1)
                    ->setStatus(Magestore_Inventorypurchasing_Model_Purchaseorder::COMPLETE_STATUS)
                    ->save();
            Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('inventorypurchasing')->__('This purchase order is completed.')
            );
            $this->_redirect('*/*/edit', array('id' => $purchaseOrderId));
        } catch (Exception $e) {
            Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('inventorypurchasing')->__('There is error while complete purchase order.')
            );
            Mage::getSingleton('adminhtml/session')->addError(
                    $e->getMessage()
            );
    ...
    protected function _preparePostData(&$data) {
        ...
        if ($this->getRequest()->getParam('id')) {
            ...
            /* Michael 201602 */
            if (Mage::getStoreConfig('inventoryplus/purchasing/require_confirmation_from_supplier') || 1)
                $data['status'] = Magestore_Inventorypurchasing_Model_Purchaseorder::PENDING_STATUS;

            $data['shipping_tax'] = Mage::getStoreConfig('inventoryplus/purchasing/shipping_includes_tax');
            $data['discount_tax'] = Mage::getStoreConfig('inventoryplus/purchasing/apply_after_discount');
            /* Michael 201602 */
        }
    }
            ...
            try {
                ...
                if ($data['payment_term_new'] && isset($paymentTerm)) {
                    ...
                    $paymentTermHistory->setData('payment_term_id', $paymentTerm->getId())
                            ->setData('time_stamp', Mage::getModel('core/date')->date())
                            ->setData('created_by', $admin)
                            ->save();
                ...
                if (isset($data['purchaseorder_products'])) {
                    ...
                    if (count($purchaseorderProductsExplodes) <= 900) {
                        Mage::helper('inventoryplus')->parseStr(urldecode($data['purchaseorder_products']), $purchaseorderProducts);
                    } else {
                        foreach ($purchaseorderProductsExplodes as $purchaseorderProductsExplode) {
                            $purchaseorderProduct = '';
                            Mage::helper('inventoryplus')->parseStr($purchaseorderProductsExplode, $purchaseorderProduct);
...
//                                    $totalAmounts += $codeArr['qty_order'] * $cost * (1 - $discount/100) * (1+ $tax/100);
    ...
    public function massStatusAction() {
        ...
        if (!is_array($purchaseOrderIds)) {
            ...
            foreach ($purchaseOrderIds as $purchaseOrderId) {
                $po = Mage::getModel('inventorypurchasing/purchaseorder')->load($purchaseOrderId);
                $po_status = $po->getStatus();
                $hasDelivery = Mage::helper('inventorypurchasing/purchaseorder')->haveDelivery($purchaseOrderId);
                if ($po_status == 6) { // PO was completed
                    Mage::getSingleton('adminhtml/session')->addError(
                            Mage::helper('inventorypurchasing')->__('Purchase order #%d cannot be changed because it was completed.', $purchaseOrderId)
                    );
                    continue;
                }
                if ($hasDelivery && $massStatus == '7') {
                    Mage::getSingleton('adminhtml/session')->addError(
                            Mage::helper('inventorypurchasing')->__('Purchase order #%d cannot be canceled because some items have been received through deliveries.', $purchaseOrderId)
                    );
                    continue;
                }
                $isPOCancelOutdate = Mage::helper('inventorypurchasing/purchaseorder')->isPOCancelOutdate($po);
                if (!$isPOCancelOutdate && $massStatus == '7') {
                    Mage::getSingleton('adminhtml/session')->addError(
                            Mage::helper('inventorypurchasing')->__('Purchase order #%d cannot be cancelled because the cancellation date has passed.', $purchaseOrderId)
                    );
                    continue;
                }
                try {
                    $po->setStatus($this->getRequest()->getParam('status'))
        ...
        Mage::getSingleton('adminhtml/session')->addSuccess(
                Mage::helper('inventorypurchasing')->__('Total %d record(s) were successfully updated', $total_saved)
        );
        $this->_redirect('*/*/index');
    }
    ...
    public function massTrashAction() {
        ...
        if (!is_array($purchaseOrderIds)) {
            ...
            foreach ($purchaseOrderIds as $purchaseOrderId) {
                ...
                if ($status == 5) {
                    Mage::getSingleton('adminhtml/session')->addError(
                            Mage::helper('inventorypurchasing')->__('Purchase Order %d cannot be moved to trash because its status is Awaiting Delivery!', $purchaseOrderId)
                    );
                } else {
                    try {
                        $model->setTrash(Magestore_Inventorypurchasing_Model_Purchaseorder::IS_TRASH)->save();
        ...
        Mage::getSingleton('adminhtml/session')->addSuccess(
                Mage::helper('inventorypurchasing')->__('Total %d record(s) have been successfully move to trash', $total_moved)
        );
        $this->_redirect('*/*/index');
    }

    public function massRecycleAction() {
        Mage::app()->cleanCache();
        $purchaseOrderIds = $this->getRequest()->getParam('purchaseorder_ids');
        $total_recycled = 0;
        if (!is_array($purchaseOrderIds)) {
        ...
        Mage::getSingleton('adminhtml/session')->addSuccess(
                Mage::helper('inventorypurchasing')->__('Total %d record(s) have been successfully restored', $total_recycled)
        );
        $this->_redirect('*/*/trash');
    }
    ...
    public function exportcsvpurchaseorderAction() {
        ...
        $fileName = 'purchase-order-' . $purchaseOrderId . '-' . Mage::helper('core')->formatDate(Mage::getModel('core/date')->date(), 'short');
    ...
    public function prepareDeliveryAction() {
        ...
        $purchaseOrder = Mage::getModel('inventorypurchasing/purchaseorder')->load($this->getRequest()->getParam('purchaseorder_id'));
        $warehouseIds = $purchaseOrder->getWarehouseId();
        $warehouseIds = explode(',', $warehouseIds);
        foreach ($warehouseIds as $warehouseId) {
            $this->getLayout()->getBlock('related_grid_serializer')->addColumnInputName('warehouse_' . $warehouseId);
        ...
        if (Mage::helper('core')->isModuleEnabled('Magestore_Inventorybarcode')) {
            /* add column to generate barcode for new products */
        ...
        try {
            if ($data = $this->getRequest()->getPost()) {
                ...
                if (isset($data['delivery_products'])) {
                    ...
                    Mage::helper('inventoryplus')->parseStr(urldecode($data['delivery_products']), $deliveryProducts);
                                            ...
                                            if ($code > 0) {
                                                ...
                                                try {
                                                    if ($warehouseProduct->getId()) {
                                                        ...
                                                        $sqlUpdateWarehouseProduct .= 'UPDATE ' . $installer->getTableName('inventoryplus/warehouse_product') . '  SET `total_qty` = ' . $qty . ', `available_qty` = ' . $newQtyAvailable . ', updated_at = \''.Mage::getModel('core/date')->date().'\' WHERE `warehouse_product_id` = ' . $warehouseProduct->getId() . ';';
                                                    } else {
                                                        $addnewProduct = true;
                                                        $warehouseInsertProduct[] = array(
                                                            'product_id' => $pId,
                                                            ...
                                                            'updated_at' => Mage::getModel('core/date')->date()
                                                        );
                ...
                $purchaseOrderProducts = Mage::getModel('inventorypurchasing/purchaseorder_product')->getCollection()
                        ->addFieldToFilter('purchase_order_id', $purchaseOrderId);
                /* Michael 201602 */
                $purchaseOrder->setStatus(Magestore_Inventorypurchasing_Model_Purchaseorder::COMPLETE_STATUS);
                ...
                foreach ($purchaseOrderProducts as $purchaseOrderProduct) {
                    if ($purchaseOrderProduct->getQtyRecieved() < $purchaseOrderProduct->getQty()) {
                        ...
                        $purchaseOrder->setStatus(Magestore_Inventorypurchasing_Model_Purchaseorder::RECEIVING_STATUS);
                    }
                ...
                Mage::getSingleton('adminhtml/session')->setFormData(false);
                $this->_redirect('adminhtml/inpu_purchaseorders/edit', array('id' => $this->getRequest()->getParam('purchaseorder_id'), 'active' => 'delivery'));
                return;
            }
        } catch (Exception $e) {
            Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
        ...
        try {
            ...
            if ($purchaseOrderProducts->getSize()) {
                ...
                foreach ($purchaseOrderProducts as $product) {
                    $sametime = strtotime(Mage::getModel('core/date')->date());
                    $results = $poProductWH->getItemData($this->getRequest()->getParam("purchaseorder_id"), $product->getProductId());
                    $qtyDeliveries = 0;
                    ...
                    foreach ($results as $result) {
                        ...
                        if ($qtyDefault > 0) {
                            ...
                            if ($qtyDefault > 0) {
                                ...
                                $sqlDeliveryWarehouseNew[] = array(
                                    'delivery_date' => Mage::getModel('core/date')->date(),
                                    'purchase_order_id' => $purchaseOrderId,
                    ...
                    $admin = Mage::getModel('admin/session')->getUser()->getUsername();
                    $delivery = Mage::getModel('inventorypurchasing/purchaseorder_delivery');
                    $delivery->setDeliveryDate(Mage::getModel('core/date')->date())
                    ...
                    Mage::dispatchEvent('save_all_delivery_after', array('warehouse_id' => $warehouseIds, 'product_id' => $product->getProductId(), 'purchase_order_id' => $purchaseOrderId, 'qty_received' => $qtyDeliveries, 'delivery' => $delivery));
                }
        ...
        try {
            if ($data = $this->getRequest()->getPost()) {
                ...
                if (isset($data['returnorder_products'])) {
                    ...
                    Mage::helper('inventoryplus')->parseStr(urldecode($data['returnorder_products']), $returnorderProducts);
                                ...
                                foreach ($codeArr as $codeId => $code) {
                                    if ($codeId != 'qty_return') {
                                        ...
                                        if ($codeId[1]) {
                                            ...
                                            $maxQtyReturn = ($maxQtyReturn >= 0) ? $maxQtyReturn : 0;
                                            if (!$code)
                                                $code = 0;
                                            ...
                                            if (floatval($code) > 0) {
                                                ...
                                                if ($stockItem->getQty() + $code <= $thresholdStockStatus) {
                ...
                Mage::getSingleton('adminhtml/session')->setFormData(false);
                $this->_redirect('adminhtml/inpu_purchaseorders/edit', array('id' => $this->getRequest()->getParam('purchaseorder_id'), 'active' => 'return'));
                return;
            }
        } catch (Exception $e) {
            Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function prepareNewReturnOrderAction() {
        ...
        if (count($warehouseIds))
            foreach ($warehouseIds as $warehouseId) {
                if (in_array($warehouseId, $availableWarehouses)) {
                    $this->getLayout()->getBlock('related_grid_serializer')->addColumnInputName('warehouse_' . $warehouseId);
                }
            }
        $this->renderLayout();
            ...
            if ($purchaseorderProducts->getSize()) {
                ...
                foreach ($purchaseorderProducts as $purchaseorderProduct) {
                    ...
                    $purchaseorderProductItem = Mage::getModel('inventorypurchasing/purchaseorder_product')
                                    ...
                                    ->setPageSize(1)->setCurPage(1)->getFirstItem();
                    $result = $poPWHresource->getQtyData($purchaseOrderId, $purchaseorderProduct->getProductId(), $purchaseorderProduct->getWarehouseId());
                    $maxQtyReturn = $result['qty_received'] - $result['qty_returned'];
                    $maxQtyReturn = ($maxQtyReturn >= 0) ? $maxQtyReturn : 0;
                    ...
                    $admin = Mage::getModel('admin/session')->getUser()->getUsername();
                    $returnNewData = array(
                        'returned_on' => Mage::getModel('core/date')->date(),
                        'purchase_order_id' => $purchaseOrderId,
                    ...
                    //minus product on system
                    $stockItem = Mage::getModel('cataloginventory/stock_item')->loadByProduct($purchaseorderProduct->getProductId());
                    if (!$stockItem->getUseConfigMinQty()) {
                        $thresholdStockStatus = $stockItem->getMinQty();
                    } else {
                        $thresholdStockStatus = Mage::getStoreConfig('cataloginventory/item_options/min_qty');
                    ...
                    $stockStatus = $stockItem->getIsInStock();
                    if ($stockItem->getQty() - $maxQtyReturn <= $thresholdStockStatus) {
                        $stockStatus = Mage_CatalogInventory_Model_Stock_Status::STATUS_OUT_OF_STOCK;
                    }
...
//                    $returnOrderProduct->delete();
                ...
                Mage::getSingleton('adminhtml/session')->setFormData(false);
                return $this->_redirect('adminhtml/inpu_purchaseorders/edit', array('id' => $this->getRequest()->getParam('purchaseorder_id'), 'active' => 'return'));
            } else {
                Mage::getSingleton('adminhtml/session')->addError(
                        Mage::helper('inventorypurchasing')->__('Unable to find return order to save!')
                );
    ...
    public function checkproductAction() {
        ...
        if (isset($purchaseorder_products)) {
            ...
            if (count($purchaseorderProductsExplodes) <= 900) {
                Mage::helper('inventoryplus')->parseStr(urldecode($purchaseorder_products), $purchaseorderProducts);
            } else {
                foreach ($purchaseorderProductsExplodes as $purchaseorderProductsExplode) {
                    $purchaseorderProduct = '';
                    Mage::helper('inventoryplus')->parseStr($purchaseorderProductsExplode, $purchaseorderProduct);
    ...
    /* add by Peter */
    public function decodeproductAction() {
        $purchaseorder_products = $this->getRequest()->getPost('products');
        if ($purchaseorder_products == 'false') {
            echo 1;
            return;
        ...
        if (isset($purchaseorder_products)) {
            ...
            if (count($purchaseorderProducts)) {
                ...
                foreach ($purchaseorderProducts as $pId => $enCoded) {
                    $codeArr = array();
                    Mage::helper('inventoryplus')->parseStr(Mage::helper('inventoryplus')->base64Decode($enCoded), $codeArr);
                    $cost =  isset($codeArr['cost_product']) ? $codeArr['cost_product'] : 0 ;
                    unset($codeArr['cost_product']);
                    unset($codeArr['tax']);
                    unset($codeArr['discount']);
                    unset($codeArr['supplier_sku']);
                    foreach ($codeArr as $codeId => $code) {
                        if($code > 0) {
                            $total_qty += $code;
                            $total_cost += $cost*$code;
                        }
                    }
                }
...
//Infomation - variable in email template (we'll use then send email successfullly)
    ...
    public function removeScanData() {
        ...
        $action = Magestore_Inventorypurchasing_Block_Adminhtml_Purchaseorder_Editdelivery_Scanbarcode::SCAN_ACTION;
     ...
     * Michael 201602
     */
    public function productNotReceiveAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('inventorypurchasing.purchaseorder.edit.tab.productnotreceive');
        $this->renderLayout();
    }

    public function productNotReceiveGridAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('inventorypurchasing.purchaseorder.edit.tab.productnotreceive');
        $this->renderLayout();
    }

    public function printreceiptforreturnedAction() {
        //new version - pdf
//        require("lib/Magestore/Pdf/html2fpdf.php");
        $purchaseOrderId = $this->getRequest()->getParam('purchaseorder_id');
        $purchaseOrderProducts = Mage::getModel('inventorypurchasing/purchaseorder_product')->getCollection()
                ->addFieldToFilter('purchase_order_id', $purchaseOrderId)
                ->addFieldToFilter('qty_returned', array('gt' => 0));
        ...
        $contents .= $this->getLayout()->createBlock('inventorypurchasing/adminhtml_purchaseorder')
                ->setPurchaseorderid($purchaseOrderId)
                ->setSqlnews($sqlNews)
                ->setTemplate('inventorypurchasing/purchaseorder/printreceiptforreturned.phtml')
                ->toHtml();
        ...
        $fileName = 'returned-items-purchase-order-' . $purchaseOrderId . '-' . Mage::helper('core')->formatDate(Mage::getModel('core/date')->date(), 'short');

        $mpdf->Output($fileName . '.pdf', 'D');
        exit;
    }

    public function printreceiptforundersellingAction() {
        $purchaseOrderId = $this->getRequest()->getParam('purchaseorder_id');
        $purchaseOrderProducts = Mage::getModel('inventorypurchasing/purchaseorder_product')->getCollection()
                ->addFieldToFilter('purchase_order_id', $purchaseOrderId);
        ...
        $contents .= $this->getLayout()->createBlock('inventorypurchasing/adminhtml_purchaseorder')
                ->setPurchaseorderid($purchaseOrderId)
                ->setSqlnews($sqlNews)
                ->setTemplate('inventorypurchasing/purchaseorder/printreceiptforunderselling.phtml')
                ->toHtml();
        ...
        $fileName = 'shortfall-items-purchase-order-' . $purchaseOrderId . '-' . Mage::helper('core')->formatDate(Mage::getModel('core/date')->date(), 'short');
