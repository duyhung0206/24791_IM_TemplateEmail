...
class Magestore_Inventorypurchasing_Block_Adminhtml_Purchaseorder_Edit_Tab_Products extends Mage_Adminhtml_Block_Widget_Grid {
        ...
        if ($this->getPurchaseOrder() && $this->getPurchaseOrder()->getId()) {
            ...
            if (($deliveries->getSize() > 0)
                || in_array($this->getPurchaseOrder()->getStatus(), array(
                                                                        Magestore_Inventorypurchasing_Model_Purchaseorder::CANCELED_STATUS,
    ...
    protected function _prepareCollection()
    {
        if ($this->_editable) {
            ...
            $productIds = array();
            /**@var Magestore_Inventorypurchasing_Model_Supplier_Product @supplierProducts */
            $supplierProducts = Mage::getModel('inventorypurchasing/supplier_product')->getCollection()
            ...
            $collection->getSelect()
                        ->joinLeft(
                            array(
                                    'supplierproduct' => $collection->getTable('erp_inventory_supplier_product')
                            ),
                            'supplierproduct.product_id = e.entity_id and supplierproduct.supplier_id IN (' . "'" . $supplier_id . "'" . ')',
                            array(
                                'cost_product' => "IFNULL(supplierproduct.cost,cost_table.value)",
            ...
            if ($this->getRequest()->getParam('id')) {
                $collection->getSelect()
                    ->joinLeft(
                        array(
                            'purchaseproduct' => $collection->getTable('erp_inventory_purchase_order_product')
                        ),
                        ...
                        array(
                            ...
                            //"cost_product_pu rchase" => "(purchaseproduct.cost)",
        ...
        if ($warehouseIds = $this->getRequest()->getParam('warehouse_ids')) {
            ...
            foreach ($warehouseIds as $warehouseId) {
                $this->addColumn('warehouse_' . $warehouseId, array(
                    ...
                    'align' => 'right',
                    'sortable' => false,
                    'renderer' => 'inventorypurchasing/adminhtml_purchaseorder_edit_tab_renderer_AvailableQty'
                ));
            }
    ...
    protected function _getSelectedProducts() {
        ...
        if ($productArrays) {
            ...
            foreach ($productArrays as $productArray) {
                Mage::helper('inventoryplus')->parseStr(urldecode($productArray), $purchaseorderProducts);
            ...
            foreach ($purchaseOrderProductImports as $productImport) {
                ...
                if ($productId) {
                    foreach ($productImport as $pImport => $p) {
                        if (!in_array($pImport, array('SKU' , 'SUPPLIER_SKU'))) {
